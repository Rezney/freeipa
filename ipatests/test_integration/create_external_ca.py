...
class ExternalCA(object):
    ...
    def create_ca(self, cn='example.test'):
        ...
        builder = builder.add_extension(
            x509.SubjectAlternativeName([x509.DNSName(unicode(cn))]),
            critical=False
            )
        ...
        root_ca = cert.public_bytes(serialization.Encoding.PEM)
        return root_ca
    ...
    def sign_csr(self, ipa_csr):
        ...
        Args:
            ipa_csr (str): CSR in PEM format.

        Returns:
            string: Signed CA in PEM format.
        """
        csr_tbs = x509.load_pem_x509_csr(bytes(ipa_csr), default_backend())
        ...
        ipa_ca = cert.public_bytes(serialization.Encoding.PEM)
        return ipa_ca
